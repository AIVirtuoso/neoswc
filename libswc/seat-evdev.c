...
handle_evdev_key(struct seat *seat, const struct input_event *ev)
{
	...
	if (ev->code >= BTN_MISC)
		pointer_handle_button(seat->base.pointer, time, ev->code,
			ev->value ? WL_POINTER_BUTTON_STATE_PRESSED : WL_POINTER_BUTTON_STATE_RELEASED);
	else {
		if (ev->code > 255)
			return;
...
handle_evdev_rel(struct seat *seat, const struct input_event *ev)
{
	...
	switch (ev->code) {
	case REL_X:
		pointer_handle_relative_motion(seat->base.pointer, time, wl_fixed_from_int(ev->value), 0);
		break;
	case REL_Y:
		pointer_handle_relative_motion(seat->base.pointer, time, 0, wl_fixed_from_int(ev->value));
		break;
	case REL_WHEEL:
		value = wl_fixed_from_int(ev->value * 10);
		pointer_handle_axis(seat->base.pointer, time, WL_POINTER_AXIS_VERTICAL_SCROLL, WL_POINTER_AXIS_SOURCE_WHEEL, value, ev->value * 120);
		break;
	case REL_HWHEEL:
		value = wl_fixed_from_int(ev->value * 10);
		pointer_handle_axis(seat->base.pointer, time, WL_POINTER_AXIS_HORIZONTAL_SCROLL, WL_POINTER_AXIS_SOURCE_WHEEL, value, ev->value * 120);
		break;
	default:
		break;
	}
...
handle_evdev_abs(struct seat *seat, const struct input_event *ev)
{
	...
	if (seat->abs_initialized)
		pointer_handle_absolute_motion(seat->base.pointer, time, seat->abs_x, seat->abs_y);
}
...
{
	return (bits[bit / (sizeof(unsigned long) * 8)] >> (bit % (sizeof(unsigned long) * 8))) & 1;
}
...
static bool
pick_best_device(const char *dir_path, const char *name_prefix, const char *name_substr,
                 bool want_keyboard, char *out, size_t out_len)
{
	DIR *dir;
	struct dirent *ent;
	bool found = false;
...
initialize_evdev(struct seat *seat)
{
	...
	if (pick_best_device("/dev/input/by-id", NULL, "event-kbd", true, kbd_path, sizeof(kbd_path)))
		kbd_dev = kbd_path;
	else if (pick_best_device("/dev/input/by-path", NULL, "event-kbd", true, kbd_path, sizeof(kbd_path)))
		kbd_dev = kbd_path;
	else if (pick_best_device("/dev/input", "event", NULL, true, kbd_path, sizeof(kbd_path)))
		kbd_dev = kbd_path;

	if (pick_best_device("/dev/input/by-id", NULL, "event-mouse", false, mouse_path, sizeof(mouse_path)))
		mouse_dev = mouse_path;
	else if (pick_best_device("/dev/input/by-path", NULL, "event-mouse", false, mouse_path, sizeof(mouse_path)))
		mouse_dev = mouse_path;
	else if (pick_best_device("/dev/input", "event", NULL, false, mouse_path, sizeof(mouse_path)))
		mouse_dev = mouse_path;
